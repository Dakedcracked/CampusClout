...
export default function LandingPage() {
  ...
  return (
    <main className="flex flex-col min-h-screen overflow-hidden">
      {/* Nav */}
      <nav className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-b border-border/60 bg-background/80 backdrop-blur-xl sticky top-0 z-20">
        <span className="text-xl font-black gradient-text tracking-tight">CampusClout</span>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          <Link href="/login" className="w-full sm:w-auto px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-text-muted hover:text-text-primary transition-colors font-medium text-center">
            Login
          </Link>
          <Link href="/register" className="btn-primary w-full sm:w-auto text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 text-center">
            Join Now →
          </Link>
        </div>
      </nav>
