              ...
              <div className="space-y-2">
                {onlineMembers.map((member) => (
                  <Link
                    ...
                    className="flex items-center gap-2 p-2 hover:bg-surface rounded-lg transition-colors cursor-pointer group"
                  >
                    <div className="relative flex-shrink-0 group-hover:opacity-80 transition-opacity">
                      {member.avatar_url ? (
                    ...
                    <span className="text-xs text-text-secondary truncate flex-1 group-hover:underline">
                      {member.username}
                    </span>
                  </Link>
                ))}
              ...
              <div className="space-y-2 opacity-60">
                {offlineMembers.map((member) => (
                  <Link
                    ...
                    className="flex items-center gap-2 p-2 hover:bg-surface rounded-lg transition-colors cursor-pointer group"
                  >
                    <div className="group-hover:opacity-80 transition-opacity flex-shrink-0">
                      {member.avatar_url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          src={member.avatar_url}
