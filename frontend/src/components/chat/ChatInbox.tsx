...
export default function ChatInbox({ myUserId, myMarketCap, autoStartUsername }: { myUserId: string; myMarketCap: number; autoStartUsername?: string | null }) {
  ...
  // Auto-start chat with user if autoStartUsername is provided
  useEffect(() => {
    if (autoStartUsername && !activeThread) {
      startChat(autoStartUsername);
    }
  }, [autoStartUsername, activeThread]);
  ...
  async function startChat(usernameOrEvent?: string | React.FormEvent) {
    let username = typeof usernameOrEvent === 'string' ? usernameOrEvent : search.trim();
    
    if (usernameOrEvent instanceof Event) {
      (usernameOrEvent as React.FormEvent).preventDefault();
    }
    ...
    setSearchError(null);
    try {
      const res = await fetch(`/api/v1/chat/threads/${username.trim().toLowerCase()}`, { method: "POST", credentials: "include" });
      const d = await res.json();
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-black text-[#f5f5f5]">Messages</h2>
          <p className="text-[#a3a3a3] text-xs font-medium">{threads.length} active threads</p>
        </div>
        <button className="text-2xl text-[#f5f5f5] hover:opacity-75 transition-opacity" title="New Message">📝</button>
      </div>
      ...
      <form onSubmit={startChat} className="mb-4">
        <div className="flex gap-2">
          <input
            ...
            className="w-full bg-[#121212] border border-[#262626] rounded-xl px-4 py-2.5 text-[14px] text-[#f5f5f5] focus:outline-none focus:border-[#404040] transition-colors"
          />
          <button type="submit" disabled={searching || !search.trim()} className="bg-white text-black font-semibold rounded-xl px-5 py-2.5 text-[14px] disabled:opacity-50 hover:bg-[#e5e5e5] transition-colors">
            {searching ? "..." : "Chat"}
          </button>
        </div>
          ...
          <AnimatePresence>
            {threads.map((t) => (
              <motion.button
                ...
                className="w-full text-left rounded-2xl p-4 flex items-center gap-4 transition-colors hover:bg-[#121212] border border-transparent hover:border-[#262626] bg-transparent"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <p className="font-bold text-[14px] text-[#f5f5f5]">
                      {t.other_user.display_name ?? `@${t.other_user.username}`}
                    </p>
                    <span className="text-[12px] text-[#a3a3a3]">{timeAgo(t.last_message_at)}</span>
                  </div>
                  <p className="text-[13px] text-[#a3a3a3] truncate mt-0.5">
