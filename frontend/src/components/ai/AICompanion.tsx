...
type Persona = "supportive" | "motivational" | "companion" | "lover" | "uncensored";
...
const PERSONAS: Record<Persona, { icon: string; label: string; desc: string; color: string }> = {
  ...
  uncensored: {
    icon: "😈",
    label: "Uncensored",
    desc: "No limits, highly explicit, completely unfiltered",
    color: "text-red-500",
  },
};
...
const GREETINGS: Record<Persona, string> = {
  ...
  uncensored: "No filters, no rules. What's on your mind? 😈",
};
...
export default function AICompanion() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/v1/ai/companion/history`, { credentials: "include" });
        if (res.ok) {
  ...
  const switchPersona = async (p: Persona) => {
    ...
    if (p === persona) return;
    try {
      const res = await fetch(`/api/v1/ai/companion/persona`, {
        method: "PATCH",
        credentials: "include",
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/v1/ai/companion/message`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
