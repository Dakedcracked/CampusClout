...
async def trending(
    ...
    campus_only: bool = Query(False),
    db: AsyncSession = Depends(get_db),
):
    # Pass viewer_id so we can include vote state in results
    viewer_id: uuid.UUID | None = None
    campus_id: uuid.UUID | None = None
    try:
        viewer_id = uuid.UUID(get_current_user_id(request))
        if campus_only:
            user_result = await db.execute(select(User.university_id).where(User.id == viewer_id))
            campus_id = user_result.scalar_one_or_none()
    except Exception:
        pass
    return await get_trending_profiles(db, limit=limit, viewer_id=viewer_id, campus_id=campus_id)
