...
async def send_otp_email(
    ...
    if not settings.MAILGUN_API_KEY or not settings.MAILGUN_DOMAIN:
        logger.warning(f"Mailgun not configured. OTP for {recipient_email}: {otp_code}")
        return False
    ...
    <html>
        <body style="font-family: Arial, sans-serif; background: #f5f5f5; padding: 20px;">
            <div style="max-width: 600px; margin: 0 auto; background: white; padding: 30px; border-radius: 8px;">
                <h2 style="color: #333; text-align: center;">Your CampusClout Login Code 🔐</h2>
                
                <p style="color: #666; font-size: 16px; margin: 20px 0;">
                    Hi <strong>{name}</strong>, your one-time password is:
                </p>
                
                <div style="text-align: center; background: linear-gradient(135deg, #a78bfa, #c9b8ff); padding: 30px; border-radius: 8px; margin: 20px 0;">
                    <p style="font-size: 42px; font-weight: bold; color: white; letter-spacing: 8px; margin: 0;">
                        {otp_code}
                    </p>
                </div>
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                data={
                    "from": f"CampusClout <{settings.SENDER_EMAIL}>",
                    "to": recipient_email,
                    "subject": f"Your CampusClout login code: {otp_code}",
                    "html": html_content,
                },
            )
