...
class PostComment(Base, TimestampMixin):
    ...
    parent_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("post_comments.id", ondelete="CASCADE"), nullable=True, index=True
    )
    content: Mapped[str] = mapped_column(Text, nullable=False)
    like_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    ...
    replies: Mapped[list["PostComment"]] = relationship(
        "PostComment", back_populates="parent_comment", foreign_keys="PostComment.parent_id", viewonly=True
    )
    parent_comment: Mapped["PostComment | None"] = relationship(
        "PostComment", back_populates="replies", remote_side=[id], foreign_keys=[parent_id], viewonly=True
    )
...
class PostShare(Base, TimestampMixin):
    """Track when a user shares a post (for analytics + engagement)."""
