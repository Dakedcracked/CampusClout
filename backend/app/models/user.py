...
class User(Base, TimestampMixin):
    ...
    university_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("universities.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    trending_pics: Mapped[list[str]] = mapped_column(JSON, default=list, server_default='[]')
    embedding: Mapped[list[float] | None] = mapped_column(JSON, nullable=True) # Vector embedding
    ...
    university: Mapped["University | None"] = relationship("University", back_populates="users", viewonly=True)
    matches_as_a: Mapped[list["Match"]] = relationship("Match", foreign_keys="Match.user_a_id", back_populates="user_a", viewonly=True)
    matches_as_b: Mapped[list["Match"]] = relationship("Match", foreign_keys="Match.user_b_id", back_populates="user_b", viewonly=True)
    
    # OTP & Tracking
    tracking_consent: Mapped["TrackingConsent | None"] = relationship(
        "TrackingConsent", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
    behavior_profile: Mapped["UserBehaviorProfile | None"] = relationship(
